...
void kvmmap(pagetable_t kernel_pagetable, uint64 va, uint64 pa, uint64 sz, int perm)
{
...
void printwalk(pagetable_t pgtbl, uint level)
{
  char *prefix;
  if (level == 1)
    prefix = "||";
  ...
  {
    pte_t pte = pgtbl[i];
    if (pte & PTE_V) // check whether this PTE is valid
    {
      uint64 pa = PTE2PA(pte);
